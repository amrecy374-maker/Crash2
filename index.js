...
async function verifyUser(ctx, next) {
    ...
    if (!isMember) {
        return ctx.replyWithPhoto(global.pp, {
            caption: "❌ Sorry, dear, you must subscribe to the channels first 👇👇",
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [{ text: "▶️ Bot usage tutorials", url: YOUTUBE_LINK }],
                    [{ text: "📷 CHANNEL Telegram 1", url: INSTAGRAM_LINK }],
                    [{ text: "🔵 CHANNEL Telegram 1", url: CHANNEL_INVITE_LINK }],
                    [{ text: "🔹GROUP Telegram", url: GROUP_LINK }],
                    [{ text: "🔄 Confirm subscription", callback_data: "check_membership" }]
                ]
            }
        });
    } else {
...
async function startXeony() {
    ...
    try {
        ...
        const username = user.username ? user.username : "mora_330";
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/mora_330" }, { text: 'CHANNEL', url: "https://t.me/zrrx4" }, { text: 'GROUP', url: "https://t.me/kkaammllk" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/mora_330" }, { text: 'GROUP', url: "https://t.me/kkaammllk" }]
                ]
            }
        });
    }
...
if (!XeonBotInc.authState.creds.registered) {
    ...
    let code = await XeonBotInc.requestPairingCode(phoneNumber, 'MORAMORA');
